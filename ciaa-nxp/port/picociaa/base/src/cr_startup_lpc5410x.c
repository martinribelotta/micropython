...
volatile struct {
	union {
		struct {
			...
			uint32_t INVSTATE:1;
			uint32_t INVPC:1;
			uint32_t res_20_23:4;
			uint32_t UNALIGNED:1;
			uint32_t DIVBYZERO:1;
			uint32_t res_27_31:6;
		} bits;
		uint32_t data;
	} CFSR;
} fault_status;

//#define FAULT_SECTION ".after_vectors"
#define FAULT_SECTION ".ramfunc"

__attribute__ ((section(FAULT_SECTION)))
void save_fault(void) {
	fault_status.CFSR.data = SCB->CFSR;
}
