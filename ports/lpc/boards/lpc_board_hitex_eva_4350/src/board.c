...
static void lcdPinConfig(void)
{
	...
	/* DC PIN */
	Chip_SCU_PinMuxSet(0x0E, 8, (SCU_MODE_PULLUP | SCU_MODE_FUNC4));
	Chip_GPIO_SetPinDIROutput(LPC_GPIO_PORT, 7, 8);
}
...
static void writeLCDReg(uint16_t addr, uint16_t data)
{
	...
	Chip_GPIO_SetPinState(LPC_GPIO_PORT, 0x06, 10, true);
	Chip_GPIO_SetPinState(LPC_GPIO_PORT, 0x06, 11, false);
...
static void readWriteTSC2046(uint8_t command, uint16_t *data)
{
	...
	tx_data[0] = command;
	Chip_GPIO_SetPinState(LPC_GPIO_PORT, 6, 10, false);
	Chip_GPIO_SetPinState(LPC_GPIO_PORT, 6, 11, true);

	Chip_GPIO_SetPinState(LPC_GPIO_PORT, 7, 16, false);
...
void Board_InitTouchController(void)
{
	...
	/* Configure SSP0 pins*/
	lcdPinConfig();
	Chip_SCU_PinMuxSet(0xF, 1, (SCU_MODE_PULLUP | SCU_MODE_FUNC4));
...
void Board_SetLCDBacklight(uint8_t Intensity)
{
	...
	Chip_GPIO_SetPinState(LPC_GPIO_PORT, 3, 8, OnOff);
}
