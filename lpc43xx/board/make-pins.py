...
SUPPORTED_AFS = { 'U': ('TX', 'RX', 'RTS', 'CTS'),
                  'SPI': ('SCK', 'MOSI', 'MISO', 'SSEL'),
                  'SSP': ('SCK', 'MOSI', 'MISO', 'SSEL'),
...
def parse_port_pin(name_str):
    ...
    m = re.match(r'P(\d+)_(\d+)', name_str)
    if m:
        port = int(m.group(1))
        gpio_bit = int(m.group(2))
        return (port, gpio_bit)
    else:
        raise ValueError("Expecting pin name")
...
def main():
    ...
    parser.add_argument(
        ...
        default="board/lpc43xx_prefix.c"
    )
    parser.add_argument(
        "-q", "--qstr",
        dest="qstr_filename",
